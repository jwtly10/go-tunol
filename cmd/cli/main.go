...
func (a *App) render() string {
	...
	for id, state := range a.tunnels {
		if state.isActive {
			...
			// We handle errors in main()... if we fail to connect we stop running the cli
			// This just support if a connection fails after the initial setup
	...
	// Admin Tools Section
	b.WriteString(color.Bold.Sprint("🔗 ADMIN TOOLS\n"))
